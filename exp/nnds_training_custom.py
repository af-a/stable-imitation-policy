...
"""
A copy of nnds_training.py, adapted to handle custom, higher-dimensional datasets
that are loaded from JSON files.
"""
...
def train_neural_policy(network: str, mode: str, data_file_path: str, 
    ...
    if plot:
        try:
            ...
            print(f'[WARN] Skipping plotting since dimension not supported')
    ...
    # Compute MSE and save predicted and reference outputs:
    vels_file_name = f'{name}-predicted_and_ref_velocities'
    mse = compute_reference_mse(nl_ds, states, state_der, save_dir=save_dir, file_name=vels_file_name)
    print(f'[INFO] MSE between reference and predicted velocities at all reference positions: {mse:.10f}')

    ''' Save the DS '''
    if save:
        print(f'[INFO] Saving model in {save_dir}/{network}/{name}.pt')
...
if __name__ == '__main__':
    ...
    train_neural_policy(args.neural_tool, args.mode, args.data_file_path,
                        args.num_epochs, args.show_plots,
